...
GameBoyAdvance.prototype.toggleAudio = function () {
    if (this.audio) {
        this.audio.muted = !this.audio.muted;
        this.audio.pause(this.audio.muted);
        this.INFO(`Audio ${this.audio.muted ? 'muted' : 'unmuted'}`);
    }
};

GameBoyAdvance.prototype.updateSpeed = function (newSpeed) {
    if (typeof newSpeed === 'number' && newSpeed > 0) {
        this.throttle = Math.floor(16 / newSpeed);
        this.INFO(`Speed updated to ${newSpeed}x (Throttle: ${this.throttle} ms)`);
    } else {
        this.WARN(`Invalid speed value: ${newSpeed}`);
    }
};


GameBoyAdvance.prototype.loadSavedata = function(data) {
